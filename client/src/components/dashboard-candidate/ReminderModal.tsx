...
export default function ReminderModal({ dayKey, monthWord, reminders, onClose, onSave, onDelete }: ReminderModalProps) {
  ...
  const toggleBase = 'flex-1 rounded-[7px] px-3.5 py-2 text-[12.5px] font-semibold transition-colors duration-150'
    ...
    >
      ...
      >
            ...
            {dayReminders.map((r) => (
              <div key={r.id} className="flex items-center gap-2.5 rounded-[9px] px-3 py-2.5" style={{ border: '1px solid var(--border)' }}>
                ...
                <button
                  ...
                  style={{ width: 22, height: 22, border: 'none', background: 'transparent', color: 'var(--text-3)', cursor: 'pointer' }}
                >
                  ×
                </button>
              </div>
        ...
        <div className="mb-[18px] flex flex-col gap-3">
          ...
          />
          <div className="flex w-fit gap-0.5 rounded-[9px] p-[3px]" style={{ border: '1px solid var(--border)' }}>
